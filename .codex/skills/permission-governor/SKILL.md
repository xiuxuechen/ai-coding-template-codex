...
description: 你是一个 AI 协作开发助手。用户请求为 Codex 设计、细化或解释执行权限策略，希望减少低风险高频确认，同时保留高风险动作的人工把关。
---
...
场景：本地开发启动 Spring Boot
分类：
- environment: local-dev
- action: service-control
- entry: project-script

建议等级：B
建议方式：允许可复用前缀授权
推荐入口：scripts/dev/run-app.sh

说明：
- 不建议直接对白名单整个 mvn
- 建议先封装为固定脚本，再对白名单脚本授权
```
